...
<header class="head">
    ...
    <div id ="pageshowwenzi"  style=" color: white;width: 20%;float: left;padding-top: 10px;font-size: 14px;margin-left: 2px;">返回</div>
...
<script>
    ...
    var id ="${id}";
    // var id = "297e9e795a3b7264015a46c3b8e70056";
...
//            document.getElementById('audio').play();
    ...
    function loadImg() {
        $.ajax({
            url:"../sysPort/getzuopinpic",
            data:{id:id},
            type:"GET",
            dataType:"json",
            success:function (data) {
                console.log(data);
                var yjgl_detail = data.result.tlkProductshowpicEntities;
                console.log(yjgl_detail);
//                $("#audio").attr("src",data.yingji.itemMusic);
                $(".headimg").attr("src", data.user.wechatInfo[0].tlkWechatuserEntity.headimgurl);
                $("#nickname").text(data.user.nickname);
                //添加封面播放图片
                if(itemCover!=null){
                    var itemCover = preImg + JSON.parse(data.result.itemZpfm)[0].path;
                    var biaoti = data.result.itemTitle;
                ...
                for(var i in yjgl_detail){
                    var imsrc =preImg+ JSON.parse(yjgl_detail[i].itemPicurl)[0].path;
                    var wenzi = yjgl_detail[i].itemDescript;
                    if(wenzi==null){
                        wenzi="";
                    }
                    var imgItem = $("<img />");
