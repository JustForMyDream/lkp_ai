...
<header class="mui-bar mui-bar-nav">
    <a  id="xiangqing" class="mui-icon mui-pull-left xiangqing">编辑</a>
    <div>
    <h1 class="mui-title" style="width: 75%">影集</h1>
    </div>
...
/*                $("#title").click(function () {
                ...
                $("#xiangqing").on("tap",function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                    window.location.href="../YINGJI/set?id="+id;
                    //window.location.href="set?id="+id;
                })
                $("#shanchu").on("click",function (e) {
                   var r = confirm("你确定要删除影集");
                    if(r == true){
                        $.ajax({
                            url:"<%=basePath%>userPort/Yingji/delete",
                            data: {id: id},
                            type: "GET",
                            dataType: "json",
                            success:function (data) {
                                window.location.href="../wechatuser/YingjiList.html";
                            },
                            error: function (e) {
                               alert("失败");
                            }
