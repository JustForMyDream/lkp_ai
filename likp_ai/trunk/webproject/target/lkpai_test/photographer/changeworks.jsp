...
<div id="coverChoose" class="mui-page">
    <div class="mui-navbar-inner mui-bar mui-bar-nav">
        ...
        <h1 class="mui-center mui-title">作品修改</h1>
    </div>
...
<script>
    ...
    function loadYingjiDat() {
        $.ajax({
            url: "<%=basePath%>sysPort/getzuopincover",
            data: {id: id},
            dataType: "json",
            success: function (data) {
                console.log(data);
                if (data.errorCode == 200) {
                  //  console.log(data.result.itemTitle);
                    var yingjidata = data.result;
                    //初始化标题描述
                   $(".describe input").val(yingjidata.itemDes);
                    $(".title input").val(yingjidata.itemTitle);
                    //初始化图片
                    for (var i = 0; i < yingjidata.tlkProductshowpicEntities.length; i++) {
                        yingji.addImg("service", imgpre + (JSON.parse(yingjidata.tlkProductshowpicEntities[i].itemPicurl))[0].path, yingjidata.tlkProductshowpicEntities[i].id,yingjidata.tlkProductshowpicEntities[i].itemDescript);
                    }
                    //初始化音乐
                    ...
                    $(".mui-backdrop").hide();
                    //初始化封面图片
                    if(yingjidata.itemZpfm!=undefined&&yingjidata.itemZpfm!=null){
                        var dataO = {"type":"service","src":imgpre+JSON.parse(yingjidata.itemZpfm)[0].path,"targetId":yingjidata.itemZpfm};
                        $("#cover").attr("src",imgpre+JSON.parse(yingjidata.itemZpfm)[0].path).attr("data-cover",JSON.stringify(dataO));
                    }else{
...
//                music: $("#musicList>li.mui-table-view-cell>a.selected").attr("data-music"),
        ...
        xhr.open("post", "../sysPort/changezuopin", true);
